...
export const ResumeController = {
    ...
    saveAdditionalInfo: async (ctx: any) => {
        try {
            ...
            if (uploaded["additional_info_file_attachment"]) {
                additionalInfoData.additional_info_file_attachment = uploaded["additional_info_file_attachment"];
            }
    ...
    followResume: async (ctx: any) => {
        ...
        try {
            const [res]: any = await pool.query("SELECT follow_resume FROM resume WHERE resume_id = ?", [resumeId]);
            var sql = "";
