...
export const ResumeController = {
    ...
    try {
      ...
      for (const [[student_id, resume_id], groupRows] of grouped.entries()) {
        ...
        const resume = {
          ...
          resume_follow_resume: first.follow_resume,
          student_id: first.student_id,
    ...
    try {
      ...
      for (const [key, data] of parsedFormData.entries()) {
        if (data instanceof File) {
          ...
          uploaded[
            key
          ] = `${process.env.API_SERVER_DOMAIN}/uploads/additional_info/${uniqueFilename}`;
          ...
          const [additionalInfoFileResult]: any = await pool.query(sqlSelect, [
            additionalInfoId,
          ]);
          const fileAttachment =
            additionalInfoFileResult[0]?.additional_info_file_attachment;
  ...
  followResume: async (ctx: any) => {
    const resumeId = ctx.params.resume_id;
    try {
      const [res]: any = await pool.query(
        "SELECT follow_resume FROM resume WHERE resume_id = ?",
        [resumeId]
      );
      var sql = "";
      if (res[0].follow_resume === "1") {
        sql = `
            UPDATE resume SET follow_resume = '0'  WHERE resume_id = ?;
        `;
      } else {
        sql = `
            UPDATE resume SET follow_resume = '1'  WHERE resume_id = ?;
        `;
      }
      const [result]: any = await pool.query(sql, [resumeId]);
      if (result.affectedRows === 0) {
        throw new Error("Failed to follow the resume or already followed.");
      }
      return {
        message: "Successfully followed the resume",
        success: true,
        status: 200,
      };
    } catch (error) {
      console.error("Error following resume:", error);
      throw error;
    }
  },
};
